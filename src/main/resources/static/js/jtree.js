		...
		genTree:function(options) {
			...
			return this.each(function(){
				...
				if (tree.size()>0) {
					...
					$(">div", node).prepend("<div class='" + (op.showSub ? op.coll : op.exp) + "'></div>"+(op.ckbox ?"<div class='ckbox " + checked + "'></div>":"")+(op.icon?"<div class='"+ (op.showSub ? op.options.folderColl : op.options.folderExp) +"'></div><input class='' style='float:left' type='checkbox' />":""));
					...
					$(">div", node).prepend("<div class='node'></div>"+(op.ckbox?"<div class='ckbox "+checked+"'></div>":"")+(op.icon?"<div class='file'></div><input class='' style='float:left' type='checkbox' />":""));
					addSpace(op.level, node);
